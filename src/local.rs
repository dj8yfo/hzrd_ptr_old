...
///
/// This garbage is pushed a thread-local queue. When enough garbage is accumulated in the
...
pub fn add_garbage(garbage: Garbage) {
    ...
    if STATE.try_with(|s| {s.borrow_mut(); }).is_err() {
        global::export_garbage(vec![garbage]);
    } else if let Ok(true) = STATE.try_with(|s| s.borrow_mut().add_garbage(garbage)) {
...
pub fn get_hazard() -> hazard::Writer {
    match STATE.try_with(|s| s.borrow_mut().get_hazard()) {
        Ok(h) => h,
        Err(e) => {
...
pub fn export_garbage() {
    ...
    // to export!
    if STATE.try_with(|s| s.borrow_mut().export_garbage()).is_ok() {
        global::tick();
    }
}
